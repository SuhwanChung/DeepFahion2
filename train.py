...
def trainStep(x, y, net, lossFn, optimizer):
    ...
    for logit, label, weight in zip(logits, labels, weights) :
        _loss = F.cross_entropy(logit, label, weight = torch.tensor(weight).cuda())
        losses.append(_loss)
    loss = sum(losses)
...
def train(trainDataLoader, validDataLoader, net, lossFn, optimizer, config) :
    ...
    for e in range(config.epochN):
        ...
        loss = float(str(np.mean(train_losses))[:4])
        ...
        loss = float(str(np.mean(val_losses))[:4])
        acc  = float(str(acc)[:4])
...
if __name__ == "__main__" :
    ...
    train(trainDataLoader, validDataLoader, net, lossFn, opt, config)